...
func (l *SQLLogger) Log(
	...
	go l.bindedSQLLogger.bindedQuery(query, alias, arg)
}
...
func (l *bindedSQLLogger) bindedQuery(
	...
	l.m.RLock()
	_, ok := l.sampled[alias]
	l.m.RUnlock()
	if ok {
		return
	}
	l.m.Lock()
	if _, ok := l.sampled[alias]; ok {
		l.m.Unlock()
		return
	}
	l.sampled[alias] = struct{}{}
	l.m.Unlock()
	if query == "" {
		return
	}
...
func (l *bindedSQLLogger) writeExplain(db queryDB, filePrefix string) error {
	f, err := os.Create(filePrefix + "_explain.sql")
	if err != nil {
		return err
	}
	defer f.Close()
...
func (l *bindedSQLLogger) explain(db queryDB, qa *queryArgs, buf *bytes.Buffer) error {
	...
	for rows.Next() {
		...
		if err != nil {
			log.Println(err)
			// mariaDB
			err := rows.Scan(&r.id, &r.selectType, &r.table, &r.joinType, &r.possibleKeys, &r.key, &r.keyLen, &r.ref, &r.rows, &r.extra)
			if err != nil {
				// mariaDB
				return err
			}
